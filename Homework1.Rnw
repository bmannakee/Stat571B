...
A <- c(3,2,4)
B <- c(5,4,7)
ttest <- t.test(A,B,alternative='less')
...
frame.table
@
      \item The mean difference between treatments in Design 1, the actual experiment, is $2.33$. There are a total of 2 experiments out of twenty that have a mean difference greater than or equal to $2.33$. So the p-value, calculated as $Pr(Difference \ge 2.33 | randomization) = \frac{2}{20} = .10$.
      \item For the two-sample t-test with $H_0: \mu_a = \mu_b$ vs. $H_a: \mu_b > \mu_a$, the t statistic is \Sexpr{round(ttest$statistic,3)} and the p-value is \Sexpr{round(ttest$p.value,3)}
      \item *.
    \end{enumerate}
  \item[2.15]
    \begin{enumerate}
...
qqnorm(sl,main='Normal probability plot',ylab='Shelf life samples')
qqline(sl)
@      
          \end{center}
        \end{figure}
